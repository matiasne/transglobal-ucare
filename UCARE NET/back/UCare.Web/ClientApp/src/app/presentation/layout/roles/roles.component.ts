...
export class RolesComponent extends CrudControllerComponent<RolesEntity> {
  ...
  public onActivarUser(data: RolesEntity) {
    if (this.onDeleteBefore(data)) {
      const dialog = this.dialog.open(AlertComponent, { data: { titulo: "Activar", label: "Esta seguro de querer activar este elemento", type: "warning" }, panelClass: ['mo-alert'], autoFocus: false });
  ...
  public onDesactivar(data: RolesEntity) {
    if (this.onDeleteBefore(data)) {
      const dialog = this.dialog.open(AlertComponent, { data: { titulo: "Desactivar", label: "Esta seguro de querer desactivar este elemento", type: "warning" }, panelClass: ['mo-alert'], autoFocus: false });
      dialog.afterClosed().subscribe(async modalAction => {
        if (modalAction) {
          this.modelService.showHourglass = true;
          data!.estado = "D";
        ...
        data: {
          ...
          header2:
            this.modelService.user?.rol === "P" ? "Para suspender un usuario gerente debes asignar otro a sus Subordinados" :
              this.modelService.user?.rol === "G" ? "Para suspender un usuario administrador debes asignar otro a sus Subordinados" :
                "",
  ...
  public onDesactivarConfirm(data: string) {
    const dialog = this.dialog.open(AlertComponent, { data: { titulo: "Desactivar el Usuario", label: "Se esta por desactivar el usuario. ¿Esta seguro de realizar esta operación?", type: "warning" }, panelClass: ['mo-alert'], autoFocus: false });
        ...
        data: {
          ...
          header2:
            this.modelService.user?.rol === "P" ? "Para eliminar un usuario gerente debes asignar otro a sus Subordinados" :
              this.modelService.user?.rol === "G" ? "Para eliminar un usuario administrador debes asignar otro a sus Subordinados" :
                "",
  ...
  public onDeleteConfirm(data: string) {
    const dialog = this.dialog.open(AlertComponent, { data: { titulo: "Borrar el Usuario", label: "Se esta por borrar el usuario. ¿Esta seguro de realizar esta operación?", type: "warning" }, panelClass: ['mo-alert'], autoFocus: false });
    dialog.afterClosed().subscribe(async (result) => {
      if (result) {
        this.modelService.showHourglass = true;
        var resul = (await lastValueFrom(this.appCrud.GetReplaceTo(this.getId(this.userToChange!) ?? "", data ?? "")));
  ...
  public onChangeUserConfirm(data: string) {
    const dialog = this.dialog.open(AlertComponent, { data: { titulo: "Cambiar el Usuario", label: "Se esta por cambiar el usuario", type: "warning" }, panelClass: ['mo-alert'], autoFocus: false });
    dialog.afterClosed().subscribe(async (result) => {
      if (result) {
        this.modelService.showHourglass = true;
