...
export abstract class CrudControllerComponent<T extends object> implements OnInit, OnDestroy, AfterContentInit {
  ...
  public onDelete(data: T): void {
    if (this.onDeleteBefore(data)) {
      const dialog = this.dialog.open(AlertComponent, { data: { titulo: "Borrar", label: "Esta seguro de querer borrar este elemento", type: "warning" }, panelClass: ['mo-alert'], autoFocus: false });
  ...
  public onCancelButton(findObject = false): void {
    const diag = this.dialog.open(AlertComponent, { data: { label: "Esta seguro de querer cancelar", type: "warning" }, panelClass: ['mo-alert'], autoFocus: false });
