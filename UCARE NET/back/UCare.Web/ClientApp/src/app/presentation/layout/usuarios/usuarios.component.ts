...
export class UsuariosComponent extends CrudControllerComponent<ConfigEntity> {

  public override formEdit: FormGroup | undefined = this.formBuilder.group(
    {
      id: [],
      usuarioActivosMaximos: [0, Validators.required],
    });
  ...
  public override onSaveDialog(data: ConfigEntity, continueInEdition = false) {
    const diag = this.dialog.open(AlertComponent, { data: { titulo: "Guardar", label: "Esta seguro de querer guardar este elemento", type: "question" }, panelClass: ['mo-alert'], autoFocus: false });
    const subscrib = diag.afterClosed().subscribe(result => {
      subscrib.unsubscribe();
      if (result) {
        this.invalidFields = null;
        this.modelService.showHourglass = true;
        this.appCrud.SaveUsuarioActivosMaximos(data).subscribe((r: ReturnData) => {
          if (!r.isError) {
  ...
  constructor(
    ...
    super(router, activatedRoute, ref, formBuilder, dialog, modelService, Constants.LAYOUT_USUARIOS, appCrud, ConfigEntity, CrudMode.Search);
  }
}
